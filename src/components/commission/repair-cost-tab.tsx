...
export function RepairCostTab() {
  ...
  return (
    <div className="space-y-6">
      <Card className="rounded-2xl shadow-md border-t-4 border-t-rose-500 overflow-hidden">
        <CardHeader className="bg-gradient-to-r from-rose-50/80 to-transparent pb-4">
          <CardTitle className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-lg">
              <div className="p-1.5 rounded-lg bg-rose-100/80"><Wrench className="h-5 w-5 text-rose-600" /></div>
              تعمیرات
            </div>
            <ExcelImport title="تعمیرات" columns={REPAIR_COST_IMPORT_COLUMNS} onImport={handleExcelImport} theme="rose" />
          </CardTitle>
          <p className="text-sm text-muted-foreground">ثبت هزینه‌های تعمیر و نگهداری مرتبط با فروش</p>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="شرح هزینه تعمیرات" className="focus-visible:ring-rose-500/30 focus-visible:border-rose-500" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-gray-700">مبلغ (ریال)</label>
              <Input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="مبلغ" dir="ltr" className="text-left font-mono tabular-nums focus-visible:ring-rose-500/30 focus-visible:border-rose-500" />
            </div>
            <Button onClick={handleAdd} disabled={!salesPersonId || !description || !amount}
              className="bg-gradient-to-r from-rose-600 to-rose-500 hover:from-rose-700 hover:to-rose-600 shadow-md shadow-rose-500/20 rounded-xl gap-1.5 transition-all duration-200 active:scale-[0.98]">
      ...
      {data.repairCosts.length > 0 && (
        <Card className="border-rose-200 bg-gradient-to-br from-rose-50 to-white rounded-2xl card-hover-lift shadow-sm">
          <CardContent className="pt-6">
            ...
            <p className="text-xl font-extrabold tracking-tight text-rose-700 animate-count-up" dir="ltr">{formatCurrency(totalRepairCost)}</p>
          </CardContent>
        </Card>
      )}
          ...
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <Table className="table-zebra">
                <TableHeader>
                  <TableRow className="bg-gradient-to-r from-rose-50/80 to-rose-50/30 hover:bg-gradient-to-r hover:from-rose-50/80 hover:to-rose-50/30">
                    <TableHead className="text-right font-semibold text-rose-800 text-xs">ردیف</TableHead>
                    <TableHead className="text-right font-semibold text-rose-800 text-xs">نام فروشنده</TableHead>
                    <TableHead className="text-right font-semibold text-rose-800 text-xs">شرح</TableHead>
                    <TableHead className="text-right font-semibold text-rose-800 text-xs">مبلغ</TableHead>
                    <TableHead className="text-right font-semibold text-rose-800 text-xs">عملیات</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                    ...
                    <TableRow key={rc.id} className="transition-all duration-150">
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <div className="w-6 h-6 rounded-full bg-gradient-to-br from-rose-400 to-pink-500 text-white text-[10px] font-bold flex items-center justify-center shadow-sm">{getSalesPersonName(rc.salesPersonId).charAt(0)}</div>
                          <span className="font-medium">{getSalesPersonName(rc.salesPersonId)}</span>
                        </div>
                      </TableCell>
