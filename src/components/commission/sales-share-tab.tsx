    ...
    {salesShares.map((ss, idx) => (
      <TableRow key={ss.id} className="transition-all duration-150">
        ...
        <TableCell className="text-right font-mono tabular-nums">
          <span dir="ltr" style={{ unicodeBidi: 'isolate' }}>{formatNumber(ss.totalSales)}</span>
        </TableCell>
        <TableCell className="text-right">{formatPercent(ss.sharePercentage)}</TableCell>
        <TableCell className="font-bold text-cyan-700 text-right font-mono tabular-nums">
          <span dir="ltr" style={{ unicodeBidi: 'isolate' }}>{formatNumber(ss.shareAmount)}</span>
        </TableCell>
        <TableCell>
